...
namespace MeasureFormula.TestHelpers
{
    public class NullablePropertyCheck
    {
        ...
        public void RunNullTestsIncludingCustomFields(object timeInvariantDto, object timeVariantDto, Func<object, object, double?[]> getUnitsFunc)
        {
            CheckForCustomFieldNullability = true;
            RunNullTests(timeInvariantDto,timeVariantDto, getUnitsFunc);
        }
        public void RunNullTestsExcludingCustomFields(object timeInvariantDto, object timeVariantDto, Func<object, object, double?[]> getUnitsFunc)
        {
            CheckForCustomFieldNullability = false;
            RunNullTests(timeInvariantDto, timeVariantDto, getUnitsFunc);
        }

        private void RunNullTests(object timeInvariantDto, object timeVariantDto, Func<object, object, double?[]> getUnitsFunc)
        {
            Func<object, double?[]> getUnitsWithoutVariantArgument = x => getUnitsFunc(timeInvariantDto, x);
            IterateThroughVariantProperties(timeVariantDto, getUnitsWithoutVariantArgument);

            Func<object, double?[]> getUnitsWithoutInvariantArgument = x => getUnitsFunc(x, timeVariantDto);
            IterateThroughTimeInvariantProperties(timeInvariantDto, getUnitsWithoutInvariantArgument);
            OutputResults();
        }

        void IterateThroughVariantProperties(object timeVariantDto, Func<object, double?[]> getUnitsFunc)
        {
            object[] timeVariantDtoArray = ((IEnumerable)timeVariantDto).Cast<object>().ToArray();
