...
namespace CustomerFormulaCode
{
	...
	public class AdditionalCostsOMAConsequence : AdditionalCostsOMAConsequenceBase
	{
		...
		{
			if (timeInvariantData.Account_32_Type.ValueAsInteger != CustomerConstants.OMAAcctID)
			{
				return null;
			}

            var operationalExpenseCalculator = new OperationalExpenseCalculator(CustomerConstants.OPEXLabourHour);

		    return InterpolatePropagate<TimeVariantInputDTO>(timeVariantData,
		        startFiscalYear,
		        months,
		        (x => operationalExpenseCalculator.AnnualAdditionalCostsFor(x.Additional_32_Costs, x.Additional_32_Hours)));
		}
