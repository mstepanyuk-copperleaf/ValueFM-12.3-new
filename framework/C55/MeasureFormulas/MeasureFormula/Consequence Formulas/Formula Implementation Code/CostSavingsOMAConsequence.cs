...
namespace CustomerFormulaCode
{
	...
	public class CostSavingsOMAConsequence : CostSavingsOMAConsequenceBase
	{
		...
		{
			if (timeInvariantData.Account_32_Type == null || timeInvariantData.Account_32_Type.ValueAsInteger != CustomerConstants.OMAAcctID) {
				return null;
			}
			return InterpolatePropagate<TimeVariantInputDTO>(timeVariantData,
			                                                 startFiscalYear,
			                                                 months, (x => (-1d * (x.Costs_32_Saved + 
			                                                                        x.Hours_32_Saved * CustomerConstants.OPEXLabourHour))));
		}
